window.addEventListener('load', (event) => {
  let listTopic = document
    .getElementById('galery-contain')
    ...
    for (let i = 0; i < paramChildren.length; i++) {
      ...
      if (currentFigure.offsetHeight > currentImg.offsetHeight) {
        ...
        currentImg.style.left =
          (currentFigure.offsetWidth - currentImg.offsetWidth) / 2 + 'px'
      } else {
        currentImg.style.width = '100%'
        currentImg.style.height = 'auto'
        currentImg.style.left = '0px'
        currentImg.style.top =
          (currentFigure.offsetHeight - currentImg.offsetHeight) / 2 + 'px'
      }
    }
  }
